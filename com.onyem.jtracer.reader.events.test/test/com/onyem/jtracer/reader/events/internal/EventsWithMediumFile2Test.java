...
public class EventsWithMediumFile2Test extends AbstractEventTest {
  ...
  public void test() {
    ...
    IInvocationEvent prevLastEvent = null;
    {
      ...
      // Continue with 5th position
      prevLastEvent = events.get(4);
    }

    // 0.5 batch of events
    {
      List<IInvocationEvent> events = eventService.getNextEvent(prevLastEvent);
      assertEvents0_5(events);
      prevLastEvent = events.get(4);
    }
  ...
  private void assertEvents0_5(List<IInvocationEvent> events) {
    Assert.assertEquals(10, events.size());
    int index = 0;
    assertEvent(InvocationEventType.MethodExit, 1, 9, events.get(index++));
    assertEvent(InvocationEventType.MethodEntry, 1, 9, events.get(index++));
  ...
  private void assertEvents1_5(List<IInvocationEvent> events) {
    Assert.assertEquals(10, events.size());
    int index = 0;
    assertEvent(InvocationEventType.MethodExit, 10, 4, events.get(index++));
    assertEvent(InvocationEventType.MethodEntry, 10, 4, events.get(index++));
  ...
  private void assertEvents2_5(List<IInvocationEvent> events) {
